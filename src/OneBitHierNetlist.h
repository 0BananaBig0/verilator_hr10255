...
class HierNetlistVisitor final: public AstNVisitor {
   ...
   private:
      ...
      // stack after finishing obtainning data only in the following three
      // cases: (1)Node has no children. For example, AstConst node.(Not pushed
      // to) (2)Node is pointed by m_nextp and its children pointed by m_opxp
      // have been visited, such as AstAssignW node.(Popped up from) (3)Node is
      // pointed by m_opxp and its children pointed by m_opxp have been
      // visited.(Popped up from) But its iterateAndNext() function will be
      ...
      // m_nextp means parent node and its children is parallel in verilog
