...
#define NUM_ASSERT_LOGIC_ARGS1(arg1)                                           \
  UASSERT(                                                                     \
      (!(arg1).isDouble() && !(arg1).isString()),                              \
      "Number operation called with non-logic (double or string) argument: '"  \
          << (arg1) << '"')
...
V3Number::V3Number(VerilogStringLiteral, AstNode *nodep, const string &str) {
  // Create a number using a verilog string as the value, thus 8 bits per
  // character. cppcheck bug - doesn't see init() resets these cppcheck: Member
  // variable 'm_sized/m_width' is not initialized in the constructor
...
void V3Number::V3NumberCreate(AstNode *nodep, const char *sourcep,
                              FileLine *fl) {
  init(nodep, 0);
  m_fileline = fl;
  ...
  if (value_startp != sourcep) { // Has a '
    ...
    if (atoi(widthn.c_str())) {
      if (atoi(widthn.c_str()) < 0 ||
          atoi(widthn.c_str()) > v3Global.opt.maxNumWidth()) {
  ...
  // Otherwise...
    ...
    if (unbased)
      isSigned(true); // Also says the spec.
  }

  // Ignore leading blanks
  while (*value_startp == '_' || isspace(*value_startp))
    value_startp++;
  ...
  if (tolower(base) == 'd') {
    // Ignore leading zeros so we don't issue too many digit errors when lots of
    // leading 0's
    while (*value_startp == '_' || *value_startp == '0')
      value_startp++;
          ...
          if (product.bitsValue(width(), 4)) { // Overflowed
            ...
            v3error(
                ...
                << ((!m_sized && !warned++)
                        ...
                        : ""));
            while (*(cp + 1))
              cp++; // Skip ahead so don't get multiple warnings
          }
        }
        olen++;
        got_01 = 1;
    ...
    if ((got_01 + got_x + got_z) > 1) {
      v3error("Mixing X/Z/? with digits not legal in decimal constant: "
              << value_startp);
    }
  } else {
    ...
    for (const char *cp = value_startp + strlen(value_startp) - 1;
      ...
      if (*cp != '_' && *cp != '0' && obit >= width()) {
        v3error("Too many digits for " << width()
                                       << " bit number: " << sourcep);
        break;
      }
      switch (tolower(base)) {
  ...
  // printf("Dump \"%s\"  CP \"%s\"  B '%c' %d W %d\n", sourcep, value_startp,
  // base, width(), m_value[0]);
}
...
V3Number &V3Number::setAllBitsX() {
  // Use setAllBitsXRemoved if calling this based on a non-X/Z input value such
  // as divide by zero
  for (int i = 0; i < words(); i++)
    m_value[i] = {~0U, ~0U};
  opCleanThis();
...
V3Number &V3Number::setAllBitsXRemoved() {
  if (!v3Global.constRemoveXs()) {
    ...
    // But after V3Unknown we have removed Xs, so use --x-assign to
    // direct-insert 0/1
...
string V3Number::ascii(bool prefixed, bool cleanVerilog) const {
  ...
  if (isDouble()) {
    ...
    if (VL_UNCOVERABLE(
            (m_value[words() - 1].m_value | m_value[words() - 1].m_valueX) &
            ~hiWordMask())) {
  ...
  if (isNull() && VL_UNCOVERABLE(!isEqZero()))
    out << "-%E-null-not-zero";
  return out.str();
}
...
bool V3Number::displayedFmtLegal(char format, bool isScan) {
  ...
  case 't':
    return true;
  case 'u':
    return true; // Packed 2-state
  case 'v':
    return true; // Strength
  case 'x':
    return true;
  case 'z':
    return true; // Packed 4-state
  case '@':
    return true; // Packed string
  case '~':
    return true; // Signed decimal
  case '*':
    return isScan; // $scan ignore argument
  default:
    return false;
  }
}

string V3Number::displayPad(size_t fmtsize, char pad, bool left,
                            const string &in) {
  string padding;
  if (in.length() < fmtsize)
    padding = string(fmtsize - in.length(), pad);
...
string V3Number::displayed(FileLine *fl, const string &vformat) const {
  ...
  case 'c': {
    if (width() > 8)
      fl->v3warn(WIDTH, "$display-like format of %c format of > 8 bit value");
  ...
  case 's': {
    // Spec says always drop leading zeros, this isn't quite right, we space
    // pad.
...
vluint64_t V3Number::toUQuad() const {
  ...
  if (isDouble())
    return static_cast<vluint64_t>(toDouble());
  for (int i = 2; i < words(); ++i) {
    if (m_value[i].m_value) {
  ...
  if (width() <= 32)
    return (static_cast<vluint64_t>(toUInt()));
...
vlsint64_t V3Number::toSQuad() const {
  if (isDouble())
    return static_cast<vlsint64_t>(toDouble());
...
uint8_t V3Number::dataByte(int byte) const {
  return (edataWord(byte / (VL_EDATASIZE / 8)) >> ((byte * 8) % VL_EDATASIZE)) &
         0xff;
}
...
bool V3Number::isBitsZero(int msb, int lsb) const {
  for (int i = lsb; i <= msb; i++) {
    if (VL_UNLIKELY(!bitIs0(i)))
      return false;
  }
  return true;
}
bool V3Number::isEqOne() const {
  if (m_value[0].m_value != 1 || m_value[0].m_valueX)
    return false;
  for (int i = 1; i < words(); i++) {
    const ValueAndX v = m_value[i];
    if (v.m_value || v.m_valueX)
...
bool V3Number::isLtXZ(const V3Number &rhs) const {
  ...
  for (int bit = 0; bit < std::max(this->width(), rhs.width()); bit++) {
    if (this->bitIs1(bit) && rhs.bitIs0(bit))
      return true;
    if (rhs.bitIs1(bit) && this->bitIs0(bit))
      return false;
    if (this->bitIsXZ(bit))
      return true;
    if (rhs.bitIsXZ(bit))
...
uint32_t V3Number::countBits(const V3Number &ctrl1, const V3Number &ctrl2,
  ...
  if (ctrl2.bitIs(0) != ctrl1.bitIs(0))
    n += countBits(ctrl2);
  if ((ctrl3.bitIs(0) != ctrl1.bitIs(0)) &&
      (ctrl3.bitIs(0) != ctrl2.bitIs(0))) {
    n += countBits(ctrl3);
  }
  return n;
}
...
V3Number &V3Number::opBitsNonX(const V3Number &lhs) { // 0/1->1, X/Z->0
  ...
  setZero();
  for (int bit = 0; bit < this->width(); bit++) {
    if (lhs.bitIs0(bit) || lhs.bitIs1(bit))
      setBit(bit, 1);
  }
  return *this;
}
...
V3Number &V3Number::opCountBits(const V3Number &expr, const V3Number &ctrl1,
...
V3Number &V3Number::opCLog2(const V3Number &lhs) {
  ...
  if (lhs.isFourState())
    return setAllBitsX();
  // IE if 4, this algorithm didn't pre-subtract 1, so we need to post-correct
  // now
...
V3Number &
V3Number::opRepl(const V3Number &lhs,
  ...
  if (!lhs.sized()) {
    v3warn(WIDTHCONCAT,
           "Unsized numbers/parameters not allowed in replications.");
  }
...
V3Number &V3Number::opRepl(const V3Number &lhs,
  ...
  if (rhsval > 8192) {
    v3warn(WIDTHCONCAT,
           "More than a 8k bit replication is probably wrong: " << rhsval);
  }
  int obit = 0;
...
V3Number &V3Number::opLogOr(const V3Number &lhs, const V3Number &rhs) {
  // i op j, 1 bit return, max(L(lhs),L(rhs)) calculation, careful need to X/Z
  // extend.
  NUM_ASSERT_OP_ARGS2(lhs, rhs);
  NUM_ASSERT_LOGIC_ARGS2(lhs, rhs);
  char outc = 0;
...
V3Number &V3Number::opAtoN(const V3Number &lhs, int base) {
  NUM_ASSERT_OP_ARGS1(lhs);
  NUM_ASSERT_STRING_ARGS1(lhs);
  UASSERT(base == AstAtoN::ATOREAL || base == 2 || base == 8 || base == 10 ||
              base == 16,
          "base must be one of AstAtoN::ATOREAL, 2, 8, 10, or 16.");

  std::string str = lhs.toString(); // new instance to edit later
  if (base == AstAtoN::ATOREAL)
    return setDouble(std::atof(str.c_str()));
...
V3Number &V3Number::opPutcN(const V3Number &lhs, const V3Number &rhs,
                            const V3Number &ths) {
  ...
  // str.len() || c == 0 when evaluating the second condition, i must be
  // positive.
  if (0 <= i && static_cast<uint32_t>(i) < lstring.length() && c != 0)
    lstring[i] = c;
...
V3Number &V3Number::opGetcN(const V3Number &lhs, const V3Number &rhs) {
  ...
  if (0 <= i && static_cast<uint32_t>(i) < lstring.length())
    v = lstring[i];
  return setLong(v);
}

V3Number &V3Number::opSubstrN(const V3Number &lhs, const V3Number &rhs,
                              const V3Number &ths) {
  NUM_ASSERT_OP_ARGS3(lhs, rhs, ths);
  NUM_ASSERT_STRING_ARGS1(lhs);
  const string lstring = lhs.toString();
  ...
  // str.len() when evaluating the third condition, j must be positive because 0
  // <= i <= j is guaranteed by the former two conditions.
  if (i < 0 || j < i || static_cast<uint32_t>(j) >= lstring.length())
    return setString("");
  // The second parameter of std::string::substr(i, n) is length, not position
  // as SystemVerilog.
  return setString(lstring.substr(i, j - i + 1));
}

V3Number &V3Number::opCompareNN(const V3Number &lhs, const V3Number &rhs,
                                bool ignoreCase) {
  NUM_ASSERT_OP_ARGS2(lhs, rhs);
  NUM_ASSERT_STRING_ARGS2(lhs, rhs);
  // SystemVerilog Language Standard does not allow a string variable to contain
  // '\0'. So C functions such as strcmp() can correctly compare strings.
  int result;
  const string lstring = lhs.toString();
...
V3Number &V3Number::opEq(const V3Number &lhs, const V3Number &rhs) {
  ...
  NUM_ASSERT_OP_ARGS2(lhs, rhs);
  if (lhs.isString())
    return opEqN(lhs, rhs);
  if (lhs.isDouble())
    return opEqD(lhs, rhs);
  char outc = 1;
...
V3Number &V3Number::opGt(const V3Number &lhs, const V3Number &rhs) {
  ...
  for (int bit = 0; bit < std::max(lhs.width(), rhs.width()); bit++) {
    if (lhs.bitIs1(bit) && rhs.bitIs0(bit))
      outc = 1;
    if (rhs.bitIs1(bit) && lhs.bitIs0(bit))
      outc = 0;
    if (lhs.bitIsXZ(bit))
      outc = 'x';
    if (rhs.bitIsXZ(bit))
...
V3Number &V3Number::opGtS(const V3Number &lhs, const V3Number &rhs) {
  ...
  char outc = 0;
  {
    ...
    if (lhs.bitIsXZ(mbit)) {
      ...
      for (int bit = 0; bit < std::max(lhs.width() - 1, rhs.width() - 1);
           bit++) {
        if (lhs.bitIs1Extend(bit) && rhs.bitIs0(bit))
          outc = 1;
        if (rhs.bitIs1Extend(bit) && lhs.bitIs0(bit))
          outc = 0;
        if (lhs.bitIsXZ(bit))
          outc = 'x';
        if (rhs.bitIsXZ(bit))
...
V3Number &V3Number::opShiftR(const V3Number &lhs, const V3Number &rhs) {
  ...
  for (int bit = 32; bit < rhs.width(); bit++) {
    if (rhs.bitIs1(bit))
      return *this; // shift of over 2^32 must be zero
  }
...
V3Number &V3Number::opShiftRS(const V3Number &lhs, const V3Number &rhs,
  ...
  for (int bit = 32; bit < rhs.width(); bit++) {
    ...
    if (rhs.bitIs1(lbits - 1))
      setAllBits1(); // -1 else 0
...
V3Number &V3Number::opAdd(const V3Number &lhs, const V3Number &rhs) {
  ...
  for (int bit = 0; bit < this->width(); bit++) {
    const int sum =
        ((lhs.bitIs1(bit) ? 1 : 0) + (rhs.bitIs1(bit) ? 1 : 0) + carry);
    if (sum & 1)
      setBit(bit, 1);
    carry = (sum >= 2);
  }
...
V3Number &V3Number::opMul(const V3Number &lhs, const V3Number &rhs) {
  ...
  if (width() <= 64) {
    ...
    for (int lword = 0; lword < lhs.words(); lword++) {
      const vluint64_t lwordval =
          static_cast<vluint64_t>(lhs.m_value[lword].m_value);
      if (lwordval == 0)
        continue;
      for (int rword = 0; rword < rhs.words(); rword++) {
        const vluint64_t rwordval =
            static_cast<vluint64_t>(rhs.m_value[rword].m_value);
        if (rwordval == 0)
          continue;
...
V3Number &V3Number::opMulS(const V3Number &lhs, const V3Number &rhs) {
  ...
  if (lhs.isNegative())
    lhsNoSign.opNegate(lhs);
  V3Number rhsNoSign = rhs;
  if (rhs.isNegative())
    rhsNoSign.opNegate(rhs);
...
V3Number &V3Number::opModDivS(const V3Number &lhs, const V3Number &rhs) {
  ...
  if (lhs.isNegative()) { // Just lhs' sign  (*DIFFERENT FROM PERL, which uses
                          // rhs sign*)
    opNegate(qNoSign);
  } else {
    opAssign(qNoSign);
...
V3Number &V3Number::opModDivGuts(const V3Number &lhs, const V3Number &rhs,
                                 bool is_modulus) {
  ...
  if (VL_UNLIKELY(vmsbp1 == 0)       // rwp==0 so division by zero.  Return 0.
    ...
    UINFO(9,
          "  opmoddiv-zero " << lhs << " " << rhs << " now=" << *this << endl);
    return *this;
  }
  ...
  if (vw == 1) { // Single divisor word breaks rest of algorithm
    ...
    for (int j = uw - 1; j >= 0; j--) {
      ...
      m_value[j].m_value =
          unw64 / static_cast<vluint64_t>(rhs.m_value[0].m_value);
      k = unw64 - (static_cast<vluint64_t>(m_value[j].m_value) *
                   static_cast<vluint64_t>(rhs.m_value[0].m_value));
    }
    UINFO(9, "  opmoddiv-1w  " << lhs << " " << rhs << " q=" << *this
                               << " rem=0x" << std::hex << k << std::dec
                               << endl);
    if (is_modulus) {
      setZero();
      m_value[0].m_value = k;
  ...
  // printf("  un="); for (int i=5; i>=0; i--) printf(" %08x",un[i]);
  // printf("\n"); printf("  vn="); for (int i=5; i>=0; i--) printf("
  // %08x",vn[i]); printf("\n"); printf("  mv="); for (int i=5; i>=0; i--)
  // printf(" %08x",m_value[i]); printf("\n");
  ...
  if (is_modulus) { // modulus
    ...
    for (int i = 0; i < vw; i++) {
      m_value[i].m_value =
          (un[i] >> s) | (shift_mask & (un[i + 1] << (32 - s)));
    }
    for (int i = vw; i < words; i++)
      m_value[i].m_value = 0;
    opCleanThis();
    UINFO(9,
          "  opmoddiv-mod " << lhs << " " << rhs << " now=" << *this << endl);
    return *this;
  } else { // division
    opCleanThis();
    UINFO(9,
          "  opmoddiv-div " << lhs << " " << rhs << " now=" << *this << endl);
    return *this;
  }
}

V3Number &V3Number::opPow(const V3Number &lhs, const V3Number &rhs, bool lsign,
                          bool rsign) {
  ...
  if (lhs.isFourState() || rhs.isFourState())
    return setAllBitsX();
  if (rhs.isEqZero())
    return setQuad(1); // Overrides lhs 0 -> return 0
  ...
  for (int bit = 0; bit < rhs.width(); bit++) {
    ...
    if (rhs.bitIs1(bit)) { // out *= power
      ...
      // UINFO(0, "pow "<<lhs<<" "<<rhs<<" b"<<bit<<" pow="<<power<<"
      // now="<<*this<<endl);
    }
  }
  return *this;
...
V3Number &V3Number::opAssign(const V3Number &lhs) {
  return opAssignNonXZ(lhs, false);
}
...
void V3Number::opCleanThis(bool warnOnTruncation) {
  ...
  if (warnOnTruncation &&
      (newValueMsb != v.m_value || newValueXMsb != v.m_valueX)) {
    // Displaying in decimal avoids hiWordMask truncation
    v3warn(WIDTH, "Value too large for " << width()
                                         << " bit number: " << displayed("%d"));
  }
  m_value[words() - 1] = {newValueMsb, newValueXMsb};
}

V3Number &V3Number::opSel(const V3Number &lhs, const V3Number &msb,
                          const V3Number &lsb) {
  NUM_ASSERT_OP_ARGS3(lhs, msb, lsb);
  NUM_ASSERT_LOGIC_ARGS1(lhs);
  NUM_ASSERT_LOGIC_ARGS2(msb, lsb);
  if (lsb.isFourState() || msb.isFourState())
    return setAllBitsX();
  return opSel(lhs, msb.toUInt(), lsb.toUInt());
}

V3Number &V3Number::opSel(const V3Number &lhs, uint32_t msbval,
                          uint32_t lsbval) {
  NUM_ASSERT_OP_ARGS1(lhs);
  NUM_ASSERT_LOGIC_ARGS1(lhs);
  setZero();
...
V3Number &V3Number::opSelInto(const V3Number &lhs, const V3Number &lsb,
                              int width) {
...
V3Number &V3Number::opRToIS(const V3Number &lhs) {
  ...
  vlsint32_t i =
      static_cast<vlsint32_t>(v); // C converts from double to vlsint32
  return setLongS(i);
}
