/*************************************************************************
  ...
  > Created Time: Sun 15 May 2022 08:59:42 AM CST
 ************************************************************************/
...
void process() {
   ...
   V3LinkDot::linkDotParamed(
      v3Global.rootp() );   // Cleanup as made new modules
   V3Error::abortIfErrors();
   ...
   if( !v3Global.opt.xmlOnly() ) {
      ...
      // Must do this after we know parameters and dtypes (as don't clone dtype
      // decls)
   ...
   if( !v3Global.opt.xmlOnly() ) {
      ...
      V3Begin::debeginAll(
         v3Global.rootp() );   // Flatten cell names, before inliner

      // Expand inouts, stage 2
      // Also simplify pin connections to always be AssignWs in prep for
      // V3Unknown
      V3Tristate::tristateAll( v3Global.rootp() );

      // Move assignments from X into MODULE temps.
      // (Before flattening, so each new X variable is shared between all scopes
      // of that module.)
   ...
   if( !v3Global.opt.xmlOnly() ) {
      ...
      // Must be before V3Task so don't need to deal with task in case value
      // compares
      ...
      // This should probably be done much later, once we have common logic
      // elimination.
      ...
      // Move assignments/sensitives into a SBLOCK for each unique sensitivity
      // list (May convert some ALWAYS to combo blocks, so should be before
      // V3Gate step.)
      V3Active::activeAll( v3Global.rootp() );

      // Split single ALWAYS blocks into multiple blocks for better ordering
      // chances
      ...
      // Gate-based logic elimination; eliminate signals and push constant
      // across cell boundaries Instant propagation makes lots-o-constant
      ...
      if( v3Global.opt.oGate() ) {
         ...
         v3info(
            "Command Line disabled gate optimization with -Og/-O0.  This may "
      ...
      // This creates lots of duplicate ACTIVES so ActiveTop needs to be after
      // this step
      ...
      if( v3Global.opt.stats() )
         V3Stats::statsStageAll( v3Global.rootp(), "PreOrder" );
      ...
      // Create tracing logic, since we ripped out some signals the user might
      // want to trace Note past this point, we presume traced variables won't
      // move between CFuncs (It's OK if untraced temporaries move around, or
      // vars "effectively" activate the same way.)
      if( v3Global.opt.trace() ) {
         V3Trace::traceAll( v3Global.rootp() );
      }

      if( v3Global.opt.stats() )
         V3Stats::statsStageAll( v3Global.rootp(), "Scoped" );
   ...
   if( !v3Global.opt.xmlOnly() ) {
      ...
      // Icache packing; combine common code in each module's functions into
      // subroutines
   ...
   if( !v3Global.opt.xmlOnly() ) {
      ...
      // Here down, widthMin() is the Verilog width, and width() is the C++
      // width Bits between widthMin() and width() are irrelevant, but may be
      // non zero.
   ...
   if( !v3Global.opt.xmlOnly() && v3Global.opt.oExpand() ) {
   ...
   if( !v3Global.opt.lintOnly() && !v3Global.opt.xmlOnly()
      ...
      // emitcInlines is first, as it may set needHInlines which other emitters
      // read
   ...
   if(
      ...
      && !v3Global.opt
             .dpiHdrOnly() ) {   // Unfortunately we have some lint checks in emitc.
      V3EmitC::emitc();
   }
   if( v3Global.opt.xmlOnly()
...
void VerilogNetlist::parseHierNet( int argc, char** argv, char** env ) {
   ...
   v3Global.opt.parseOpts(
      new FileLine( FileLine::commandLineFilename() ), argc - 1, argv + 1 );
   ...
   if( v3Global.opt.skipIdentical().isTrue()
       && V3File::checkTimes( v3Global.opt.makeDir() + "/"
                                 + v3Global.opt.prefix() + "__verFiles.dat",
                              argString ) ) {
   ...
   V3Os::unlinkRegexp( v3Global.opt.makeDir(),
                       v3Global.opt.prefix() + "_*.dot" );
   V3Os::unlinkRegexp( v3Global.opt.makeDir(),
                       v3Global.opt.prefix() + "_*.txt" );
   ...
   genHierNet();
   // Final steps
   V3Global::dumpCheckGlobalTree(
      "final", 990, v3Global.opt.dumpTreeLevel( __FILE__ ) >= 3 );
   V3Error::abortIfWarnings();

   if( v3Global.opt.makeDepend().isTrue() ) {
      V3File::writeDepend( v3Global.opt.makeDir() + "/" + v3Global.opt.prefix()
                           + "__ver.d" );
   }
   if( v3Global.opt.skipIdentical().isTrue()
       || v3Global.opt.makeDepend().isTrue() ) {
      V3File::writeTimes( v3Global.opt.makeDir() + "/" + v3Global.opt.prefix()
                             + "__verFiles.dat",
                          argString );
   }
   if( v3Global.opt.protectIds() ) {
      VIdProtect::writeMapFile( v3Global.opt.makeDir() + "/"
                                + v3Global.opt.prefix() + "__idmap.xml" );
   }
