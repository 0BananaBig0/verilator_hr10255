...
void process()
{
  ...
  if(!v3Global.opt.xmlOnly())
  {
    ...
    // Gate-based logic elimination; eliminate signals and push constant across
    // cell boundaries Instant propagation makes lots-o-constant reduction
    // possibilities.
    ...
    {
      v3info("Command Line disabled gate optimization with -Og/-O0.  This may "
  ...
  if(!v3Global.opt.xmlOnly() && !v3Global.opt.dpiHdrOnly())
