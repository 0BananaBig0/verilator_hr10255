...
void HierNetlistVisitor::visit(AstNetlist *nodep) {
   ...
   freeContainerBySwap(_multipleBitsAssignStatementTmp.lValue().values());
   ...
   freeContainerBySwap(_multipleBitsRefVarTmp.values());
};
...
void HierNetlistVisitor::visit(AstModule *nodep) {
   ...
   if(_theTimesOfVisit == 1 && !inLibrary) {
      ...
      auto visitAstModuleAndAstVar = [this, &curModuleName, &nodep]() {
         ...
         iterateChildren(nodep);
         _hierNetlist[_curModuleIndex].totalInouts(_curPortDefIndex);
         // visit inout
         _theTimesOfVisitAstVar = 2;
         iterateChildren(nodep);
         _hierNetlist[_curModuleIndex].totalInoutsAndInputs(_curPortDefIndex);
         // visit output
...
void HierNetlistVisitor::visit(AstNodeAssign *nodep) {
   if(_theTimesOfVisit == 5) {
      ...
      assigns.resize(
         assignsIndex + lEnd -
         _multipleBitsAssignStatementTmp.lValue().refVarRange().start + 1);
      ...
      for(auto &rValue: _multipleBitsAssignStatementTmp.rValue()) {
         if(rValue.refVarName() == "") { // rValue is a const value, X or Z.
            auto &rWidth = rValue.width();
            uint32_t position;
            uint32_t valuesSize = rValue.values().size();
            // Store rValue
            bitSlicedAssignStatementTmp.rValue.refVarDefIndex = UINT32_MAX;
            while(valuesSize > 0) {
               auto &value = rValue.values()[valuesSize - 1];
               if(valuesSize == rValue.values().size())
                  position = rWidth - 32 * (valuesSize - 1) - 1;
               else
                  position = 31;
               while(position != UINT32_MAX) {
                  // Store rValue
                  bitSlicedAssignStatementTmp.rValue.valueAndValueX =
                     getOneBitValueFromDecimalNumber(value.value,
                                                     value.valueXOrZ, position,
                                                     rValue.hasXOrZ());
         ...
         } else { // rValue is a input, output, wire or inout.
            {
...
void HierNetlistVisitor::visit(AstPin *nodep) {
   ...
   auto &portDefIndex =
      _hierNetlist[curSubModuleIndex].portNameMapPortDefIndex()
   ...
   for(int i = _multipleBitsPortAssignmentTmp.multipleBitsRefVars().size() - 1;
      ...
      if(mRefVar.refVarName() == "") {
         ...
         for(auto &value: mRefVar.values()) {
            if(&value == &mRefVar.values().back())
               positionLimit = rWidth - 32 * (mRefVar.values().size() - 1);
            else
               positionLimit = 32;
            position = 0;
            while(position < positionLimit) {
               refVar.valueAndValueX = getOneBitValueFromDecimalNumber(
                  value.value, value.valueXOrZ, position, mRefVar.hasXOrZ());
...
void HierNetlistVisitor::visit(AstSel *nodep) {
   ...
   iterateChildren(nodep);
   if(_isAssignStatement) { // Now, AstSel is a child or a descendant of
                            // AstNodeAssign
      if(_isAssignStatementLvalue) { // Now, AstSel is a child of AstNodeAssign
         _multipleBitsAssignStatementTmp.lValue() = _multipleBitsRefVarTmp;
         _isAssignStatementLvalue                 = false;
      } else { // Now, AstSel is a child of AstNodeAssign or AstExtend or
               // AstConcat
...
void HierNetlistVisitor::visit(AstVarRef *nodep) {
   ...
   if(_whichAstSelChildren) { // Now, AstVarRef is a child of AstSel
      _whichAstSelChildren++;
      if(_isAssignStatement && (nodep->lvalue())) {
         _isAssignStatementLvalue = true;
      }
   } else {
      _multipleBitsRefVarTmp.hasXOrZ(false);
      ...
      _multipleBitsRefVarTmp.width(_multipleBitsRefVarTmp.refVarRange().end -
                                   _multipleBitsRefVarTmp.refVarRange().start +
                                   1);
      if(_isAssignStatement) { // Now, AstVarRef is a child of AstNodeAssign or
                               // a
         // descendant of AstNodeAssign
         if(nodep->lvalue()) { // Now, AstVarRef is a child of AstNodeAssign
            _multipleBitsAssignStatementTmp.lValue() = _multipleBitsRefVarTmp;
         } else { // Now, AstVarRef is a child of AstNodeAssign or AstExtend or
...
void HierNetlistVisitor::visit(AstExtend *nodep) {
   ...
   _multipleBitsRefVarTmp.values().resize(1);
   _multipleBitsRefVarTmp.values()[0].value     = 0;
   _multipleBitsRefVarTmp.values()[0].valueXOrZ = 0;
   _multipleBitsRefVarTmp.hasXOrZ(false);
   _multipleBitsRefVarTmp.width(std::move(extendWidth));
   if(_isAssignStatement) { // Now, AstExtend is a child of AstNodeAssign or
                            // AstConcat
      _multipleBitsAssignStatementTmp.rValue().push_back(
         _multipleBitsRefVarTmp);
   } else { // Now, AstExtend is a child of AstPin or AstConcat
      _multipleBitsPortAssignmentTmp.multipleBitsRefVars().push_back(
         _multipleBitsRefVarTmp);
   }
   iterateChildren(nodep);
...
void HierNetlistVisitor::visit(AstExtendS *nodep) {
   ...
   _multipleBitsRefVarTmp.refVarName("");
   _multipleBitsRefVarTmp.values().resize(1);
   _multipleBitsRefVarTmp.values()[0].value     = (1 << extendSWidth) - 1;
   _multipleBitsRefVarTmp.values()[0].valueXOrZ = 0;
   _multipleBitsRefVarTmp.hasXOrZ(false);
...
void HierNetlistVisitor::visit(AstReplicate *nodep) {
   iterateChildren(nodep);
   if(_isAssignStatement) { // Now, AstReplicate is a child of AstNodeAssign or
                            // AstExtend or AstConcat
      uint32_t replicateTimes =
         _multipleBitsAssignStatementTmp.rValue().back().values()[0].value;
   ...
   } else { // Now, AstReplicate is a child of AstPin or AstExtend or AstConcat
      uint32_t replicateTimes =
...
void HierNetlistVisitor::visit(AstConst *nodep) {
   if(_whichAstSelChildren == 2) { // Now, AstConst is a child of AstSel
      _whichAstSelChildren++;
      _multipleBitsRefVarTmp.refVarRange().start = nodep->num().getValue32();
   } else if(_whichAstSelChildren == 3) { // Now, AstConst is a child of AstSel
      _multipleBitsRefVarTmp.width(nodep->num().getValue32());
      _multipleBitsRefVarTmp.refVarRange().end =
         _multipleBitsRefVarTmp.refVarRange().start +
         _multipleBitsRefVarTmp.width() - 1;
      _multipleBitsRefVarTmp.hasXOrZ(false);
   } else { // Now, AstConst is a rValue of assign statement or refValue of a
      ...
      ConstValueAndXOrZ constValueAndXOrZ;
      _multipleBitsRefVarTmp.values().clear();
      for(uint32_t i = 0; i < ceil(_multipleBitsRefVarTmp.width() / 32.0);
          i++) {
         constValueAndXOrZ.value     = values[i];
         constValueAndXOrZ.valueXOrZ = valueXs[i];
         _multipleBitsRefVarTmp.values().push_back(constValueAndXOrZ);
         if(constValueAndXOrZ.valueXOrZ > 0 &&
            _multipleBitsRefVarTmp.hasXOrZ() ==
               false) { // Now, the const value has value x or z.
            _multipleBitsRefVarTmp.hasXOrZ(true);
         }
      }
      if(_isAssignStatement) { // Now, AstConst is a child of AstNodeAssign or
                               // AstConcat or
         // AstReplicate
         _multipleBitsAssignStatementTmp.rValue().push_back(
            _multipleBitsRefVarTmp);
      } else { // Now, AstConst is a child of AstPin or AstConcat or
               // AstReplicate
         _multipleBitsPortAssignmentTmp.multipleBitsRefVars().push_back(
            _multipleBitsRefVarTmp);
      }
...
char HierNetlistVisitor::getOneBitValueFromDecimalNumber(
   const uint32_t &value, const uint32_t &valueX, const uint32_t &position,
...
bool HierNetlistVisitor::isAnEmptyStdCellInJson(
   const std::string &stdCellName) {
