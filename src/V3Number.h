...
// Holds a few entries of ValueAndX to avoid dynamic allocation in std::vector
// for less width of constants
...
private:
  ...
  static constexpr size_t m_inlinedSize =
      2; // Can hold 64 bit without dynamic allocation
...
public:
  // CONSTRUCTORS
  V3NumberData() {
    for (size_t i = 0; i < m_inlinedSize; ++i)
      m_inlined[i] = {0, 0};
  }

  // METHODS
  void ensureSizeAtLeast(size_t s) {
    if (VL_UNLIKELY(s > m_data.size() + m_inlinedSize))
      m_data.resize(s - m_inlinedSize);
  }
...
private:
  ...
  char bitIsExtend(int bit, int lbits) const {
    // lbits usually = width, but for C optimizations width=32_bits, lbits =
    // 32_or_less
    if (bit < 0)
      return '0';
...
public:
  bool bitIs0(int bit) const {
    if (bit < 0)
      return false;
    if (bit >= m_width)
      return !bitIsXZ(m_width - 1);
    const ValueAndX v = m_value[bit / 32];
    return ((v.m_value & (1UL << (bit & 31))) == 0 &&
            !(v.m_valueX & (1UL << (bit & 31))));
  }
...
private:
  ...
  V3Number &opModDivGuts(const V3Number &lhs, const V3Number &rhs,
                         bool is_modulus);
...
public:
  ...
  V3Number(AstNode *nodep, const char *sourcep) {
    V3NumberCreate(nodep, sourcep, nullptr);
  }
  class FileLined {
  }; // Fileline based errors, for parsing only, otherwise pass nodep
...
private:
  ...
  static string displayPad(size_t fmtsize, char pad, bool left,
                           const string &in);
...
public:
  ...
  V3Number &
  setMask(int nbits); // IE if nbits=1, then 0b1, if 2->0b11, if 3->0b111 etc

  // ACCESSORS
  string ascii(bool prefixed = true, bool cleanVerilog = false) const;
  string displayed(AstNode *nodep, const string &vformat) const;
  static bool displayedFmtLegal(char format,
                                bool isScan); // Is this a valid format letter?
  int width() const { return m_width; }
  int widthMin()
      const; // Minimum width that can represent this number (~== log2(num)+1)
  ...
  uint32_t countBits(const V3Number &ctrl1, const V3Number &ctrl2,
                     const V3Number &ctrl3) const;
  uint32_t countOnes() const;
  uint32_t mostSetBitP1()
      const; // Highest bit set plus one, IE for 16 return 5, for 0 return 0.
  ...
  V3Number &opCountBits(const V3Number &expr, const V3Number &ctrl1,
  ...
  V3Number &opSel(const V3Number &lhs, const V3Number &msb,
                  const V3Number &lsb);
  ...
  V3Number &opPow(const V3Number &lhs, const V3Number &rhs, bool lsign = false,
                  bool rsign = false);
  V3Number &opPowSU(const V3Number &lhs,
                    const V3Number &rhs); // Signed lhs, unsigned rhs
  V3Number &opPowSS(const V3Number &lhs,
                    const V3Number &rhs); // Signed lhs, signed rhs
  V3Number &opPowUS(const V3Number &lhs,
                    const V3Number &rhs); // Unsigned lhs, signed rhs
  ...
  V3Number &opShiftRS(const V3Number &lhs,
                      const V3Number &rhs, // Arithmetic w/carry
                      uint32_t lbits);
  ...
  V3Number &opPutcN(const V3Number &lhs, const V3Number &rhs,
                    const V3Number &ths);
  V3Number &opGetcN(const V3Number &lhs, const V3Number &rhs);
  V3Number &opSubstrN(const V3Number &lhs, const V3Number &rhs,
                      const V3Number &ths);
  V3Number &opCompareNN(const V3Number &lhs, const V3Number &rhs,
                        bool ignoreCase);
